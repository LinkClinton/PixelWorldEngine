...
关于纹理管理器，主要是将我们原本分散的纹理组合到一起，以方便我们加快渲染速度。在1.0版本中我们需要自己管理纹理的合并，而在1.03beta中，我们并不需要自己管理，而是交给了程序自己进行管理。因此我们只需要将纹理加入到纹理管理其中去即可，但要注意，纹理管理器有支持的纹理格式，因此创建的时候需要指定其支持的纹理格式

```C++
    auto textureManager = new TextureManager(Application, PixelFormat);
```

其第一个参数是应用程序的指针，第二个参数就是纹理格式，通常我们可以指定为`R8G8B8A8`。

而如果要注册纹理，那么就只需要纹理要注册的渲染编号以及纹理即可，如果要卸载纹理，那么只需要渲染编号

```C++
    TextureManager::RegisterTexture(RenderObjectID, Texture);
    TextureManager::UnRegisterTexture(RenderObjectID);
```

## 地图

地图类型仅仅只是描述我们的地图的属性，例如一个地图块使用的渲染编号，它的不透明度。
...
我们可以使用构造函数来创建我们的地图，我们可以预先设置好地图数据再创建，也可以创建好后再去设置地图数据。
